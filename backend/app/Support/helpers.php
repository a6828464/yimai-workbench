 ...
 *
 * 映射值（`1`=团课 / `2`=小班 / `3`=私教）于本任务**修正过一次方向**。原实现写
 * 反了（`2 => private`、`3 => small`），而 t26 曾判「用现有证据无法判定」——
 * 该判定当时**成立但前提不全**：t26 被禁止读 `随心瑜后台解读/notes.md`
 * （实地勘察的第一手笔记），因而漏掉了决定性的数字↔内部名绑定。
 *
 * 判定依据（现已确认，用户已确认口径）：
 *  - `随心瑜后台解读/notes.md:425-426`（**直接绑定数字**，非仅 UI 文案）：
 *    「课程类型自定义名称（仅影响约课页展示）：团课→显示『精品团课』；
 *     精品课→显示『私教小班』；私教课→显示『定制私教』」⇒ `course_type` =
 *    1(团课) / 2(精品课) / 3(私教课)。
 *  - 《随心瑜后台完整解读》`:262-268` 独立复述同一绑定，并给字段级佐证：
 *    扣费流水 `course_type_name` **回显内部名**（如「精品课」），品牌消费统计的
 *    「消费项目」枚举为 预约团课/预约精品课/预约私教。
 *  - 旁证：`notes.md` 内部三分类「团课/精品课/私教课」在 15 处独立页面出现
 *    （排课/课时费/评价筛选/教练分析/课程分析/模板消息/统计枚举等），
 *    说明这是随心瑜稳定的内部枚举，不是某一个页面的措辞。
 *
 * 因此：**「私教」= `3`（对客显示「定制私教」）、「小班」= `2`（对客显示
 * 「私教小班」）**。`KIND_LABELS` 的展示语义（private=私教/small=小班/group=团课）
 * 本身是对的，修正的只是「哪个 `course_type` 归到哪个内部值」。
 *
 * 历史数据影响：`ky_bookings.course_kind` 是同步时按旧映射落盘的，故新增迁移
 * `2026_09_21_000003_recompute_course_kind_after_mapping_fix` 重算回补——
 * 不回补的话，修正本函数只会让**新**数据正确，历史行继续反向。
 *
 ...
 *  - `group` 是三分里的**剩余类**：能确定是私教的行上游会带 `course_type=3`，
 *    小班带 `2`，所以「没带课型」时按非私教处理，与「private 必须有显式证据」
 *    的权限原则一致。
 *
...
function courseKindFrom(?string $courseType): string
{
    return match ((string) $courseType) {
        // 1=团课（对客显示「精品团课」）、2=精品课（对客显示「私教小班」）→ 工作台「小班」、
        // 3=私教课（对客显示「定制私教」）→ 工作台「私教」。
        '2' => 'small',
        '3' => 'private',
        '1' => 'group',
        default => 'group',
