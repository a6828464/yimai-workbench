...
function ym_ok(bool $b): string
{
    return $b ? '<span style="color:#1d9a5b">✔ 通过</span>' : '<span style="color:#d43c33">✘ 不满足</span>';
}
function ym_h(string $s): string
{
    return htmlspecialchars($s, ENT_QUOTES, 'UTF-8');
}
function ym_post(string $k, string $d = ''): string
{
    return isset($_POST[$k]) ? trim((string) $_POST[$k]) : $d;
}
function ym_env(string $s): string
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $allPass && ym_post('action') === 'install') {
    ...
    if ($result !== 'FAIL') {
        try {
            ...
            $rawEnvKeys = ['APP_DEBUG', 'DB_PORT', 'SESSION_LIFETIME'];
            foreach ([
                ...
                $env .= "{$k}=".(in_array($k, $rawEnvKeys, true) ? (string) $v : ym_env((string) $v))."\n";
            }
            if ($kyPhone !== '') {
