<template>
  <div class="mx-auto pt-5 mb-5 space-y-5">
    <!--
      ...
      超管执行完升级后需要一条能一眼看到、可直接转达门店的操作说明，藏在长日志里容易漏。
    -->
    <ElAlert type="warning" :closable="false" class="!rounded-lg">
      <template #title>升级后请各门店重新开启一次 H5 分享</template>
      <template #default>
        <div class="text-sm leading-6">
          本次升级把对外分享码从「前端固定常量」改为「服务端随机签发」，历史遗留的旧分享码一律不再对外下发。
          因此升级完成后，<strong>各门店的 H5 分享链接会暂时失效</strong>——谈单工具里开关仍显示「分享中」，
          但客户打开会提示「链接已失效或已停用」。
          <br />
          处理方式：请各门店进入
          <strong>谈单工具 → 把 H5 分享开关停用后再开启一次</strong>，即可获得新的分享码，新链接立即可用。
          该页检测到这种状态时也会标红提示「分享码已失效，请重新开启分享」。
          <br />
          原因：旧分享码由前端编译期常量决定，任何拿到前端产物的人都能读出它并直接请求公开接口，
          学员案例（含健康、生育类描述）会随接口原文出网；改为服务端签发并记录来源后，
          来源不可信的码不再放行——这正是本次加固的目的，代价是历史链接需各门店重新发布一次。
        </div>
      </template>
    </ElAlert>
