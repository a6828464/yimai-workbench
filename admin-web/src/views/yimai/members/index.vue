...
<script setup lang="ts">
   ...
   * 4. 第 5 条口径「超 6 个月未到店」由后端 `customerDecision` 判（2026-09-25 补充），
   *    决定「谁在清单里」；本列只做展示层过滤，不重复判 last_visit——
   *    清单里可能出现「卡符合但近期来过」的会员吗？不会：后端已把它排除在清单外。
   *    本列对清单内会员展示其全部沉睡卡（即便某会员 last_visit 未知）。
   *
