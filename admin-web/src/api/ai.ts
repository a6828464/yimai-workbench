...
import { saveMarketingHistory } from './my'
  ...
  try {
    ...
    if (saved) {
      Object.assign(store.config, {
        ...
        temperature:
          typeof saved.temperature === 'number' ? saved.temperature : store.config.temperature,
...
/** 品牌与城市全系统写死，不再由用户填写 */
export const BRAND_NAME = '一麦'
export const BRAND_CITY = '宁波'

/** 身份角色（一麦组织岗位，v3.1.31 起删除「健身教练」） */
export const PERSONA_ROLES = ['会籍顾问', '全职老师', '门店前台', '管理层', '主理人']

/** 专业方向（个人中心与发圈人设共用，支持自定义添加） */
...
/** 小红书选题模板（v3.1.31 新增，与朋友圈同款分类逻辑，含销售类） */
export const XHS_CATEGORIES: Record<string, string[]> = {
  干货教程: [
    '新手入门动作',
    '办公室拉伸',
    '呼吸与核心',
  ...
  案例故事: ['学员3个月变化', '体态对比记录', '会员私信', '复训故事', '从疼痛到无痛'],
  同城到店: ['宁波门店日常', '体验课流程', '体态评估现场', '环境器械展示', '宁波同城约练'],
  销售转化: [
    '体验课招募',
    '新客福利',
    '限时优惠',
...
function personaText(p: MomentsPersona): string {
  return [
    ...
    p.specialties.length && `专业方向：${p.specialties.join('、')}`,
    `品牌：${p.brand || BRAND_NAME}`,
    `城市：${p.city || BRAND_CITY}`,
...
function buildMomentsMessages(input: MomentsInput): ChatMessage[] {
  ...
  const system = [
    ...
    '严格输出 JSON（不要代码块、不要多余解释）：{"content": string, "reply": string}。',
    `content 为朋友圈正文，长度控制在${len.min}-${len.max}字，`,
    input.multiLine ? '使用多短行分段排版。' : '使用自然段落。',
    input.emojiLevel === '不使用' ? '正文不使用表情符号。' : `表情符号使用：${input.emojiLevel}。`,
    'reply 为发布后自己评论的第一条回复建议（统一回复/评论区补充）：40-80字，口吻自然不硬广，' +
      '承接正文给一个具体下一步（约体验、做评估、看档期、私信领福利等），与本次目标呼应。'
  ].join('\n')
...
function buildXhsMessages(input: XhsInput): ChatMessage[] {
  ...
  const system = [
    `你是小红书瑜伽普拉提赛道的内容策划，为「${BRAND_NAME}」（${BRAND_CITY}）的账号生成笔记草稿。`,
    '写作规范：标题带搜索感/场景感/问题感；正文开头快速命中目标读者，中段给出具体可收藏的方法信息，结尾轻转化；',
    '始终以教练/服务者视角，不伪造会员第一人称经历；不含医疗承诺与虚假效果保证。',
    '严格输出 JSON：{"title": string, "content": string, "tags": string[], "reply": string}，' +
      'tags 为话题标签数组（含#号），需覆盖行业词、问题词、人群词' +
      (p.localFocus ? '、同城词' : '') +
      '和本次选题词；' +
      'reply 为发布后置顶评论/首条自评：30-60字，给出下一步动作（私信、约评估、留言答疑等），与转化方向一致。'
  ].join('\n')

  const user = [
    `账号定位：${p.ipType}${p.accountName ? `（账号名：${p.accountName}）` : ''}`,
    `身份角色：${p.role || '全职老师'}${p.gender ? `/${p.gender}` : ''}${p.age ? `/${p.age}` : ''}`,
    `品牌：${BRAND_NAME}`,
    `城市：${BRAND_CITY}${p.localFocus ? '（强化同城属性，自然融入宁波、到店、体验课等词）' : ''}`,
    p.audiences.length && `目标客群（最多4项）：${p.audiences.slice(0, 4).join('、')}`,
    p.strengths.length && `专业擅长：${p.strengths.join('、')}`,
    `表达风格：${p.style}；默认转化方向：${p.conversion}`,
    input.category && `选题分类：${input.category}`,
...
export async function fetchAvailableModels(baseUrl: string, apiKey: string): Promise<string[]> {
  ...
  const d = await apiPost<{ models?: string[]; code?: number; message?: string }>('/ai/models', {
    baseUrl,
    apiKey
  })
...
function parseJsonLoose(
  text: string
): { title: string; content: string; tags: string[]; reply: string } | null {
  ...
  try {
    ...
    if (obj.title && obj.content) {
      return {
        ...
        reply: obj.reply ? String(obj.reply).trim() : ''
      }
    }
  } catch {
    /* ignore */
...
/** 朋友圈 JSON 解析失败时的宽容处理：整段视为正文，首评留空由本地模板兜底 */
function parseMomentsLoose(text: string): { content: string; reply: string } {
  const m = text.match(/\{[\s\S]*\}/)
  if (m) {
    try {
      const obj = JSON.parse(m[0])
      if (obj.content) {
        return { content: String(obj.content), reply: obj.reply ? String(obj.reply).trim() : '' }
      }
    } catch {
      /* ignore */
    }
...
function fallbackMoments(input: MomentsInput): { content: string; reply: string } {
  const lines: string[] = []
  ...
  lines.push(`我们在${p.city || BRAND_CITY}·${p.brand || BRAND_NAME}，想一起练的评论区或私信找我。`)
  const reply =
    input.goal.includes('续课') || input.goal.includes('升单')
      ? '统一回复：老会员续课本周有专属方案，想延续节奏的私信我，帮你把下阶段计划排好。'
      : input.category === '销售转化' || input.goal === '招募体验课'
        ? '统一回复：近期还有少量体验名额，宁波的姐妹私信「体验」，我按你的时间约一节课。'
        : '统一回复：最近问动作细节的姐妹比较多，与其自己瞎练，不如到店做一次评估，我来帮你看问题。'
  return { content: lines.join('\n\n'), reply }
}

function fallbackXhs(input: XhsInput): {
  title: string
  content: string
  tags: string[]
  reply: string
} {
  const topic = input.topic || input.profile.strengths[0] || '瑜伽日常'
  return {
    ...
    content: [
      ...
      '',
      input.profile.conversion !== '不主动转化'
        ? '想来体验的同学，可以私信我约一节体验课。'
        : '有问题评论区聊，看到都会回。'
    ].join('\n'),
    tags: [
      ...
      `#${topic}`,
      '#体态改善',
      input.profile.localFocus ? `#${BRAND_CITY}瑜伽` : '',
      '#运动日常'
    ].filter(Boolean),
    reply:
      input.profile.conversion === '不主动转化'
        ? '有问题评论区聊，看到都会回～'
        : `宁波的姐妹想改善${topic}，可以私信我约一次体态评估，先看问题再给方案。`
  }
}
...
function auditGenerate(platform: string, topic: string, source: 'llm' | 'fallback') {
  useYimaiStore().writeAudit(
    ...
    `${platform} · ${topic || '自由创作'}`,
    '双店',
    `生成方式：${source === 'llm' ? '大模型API' : '本地模板（未配置AI或调用失败）'}；主题[${topic || '无'}]`
  )
}

export async function generateMomentsCopy(
  input: MomentsInput
): Promise<{ content: string; reply: string; source: 'llm' | 'fallback'; warning?: string }> {
  const store = useAiConfigStore()
  let content = ''
  let reply = ''
  ...
  if (store.isReady()) {
    try {
      ...
      const fb = fallbackMoments(input)
      content = fb.content
      reply = fb.reply
    }
  } else {
  ...
  saveMarketingHistory({
    platform: '朋友圈',
    title: input.topic || content.slice(0, 20),
    content,
    reply,
    source
  })
...
export async function generateXhsNote(input: XhsInput): Promise<{
  ...
  const store = useAiConfigStore()
  let result = { title: '', content: '', tags: [] as string[], reply: '' }
  let source: 'llm' | 'fallback' = 'fallback'
  let warning: string | undefined
  if (store.isReady()) {
    try {
  ...
  saveMarketingHistory({
    platform: '小红书',
    title: result.title || input.topic || result.content.slice(0, 20),
    content: [result.title, '', result.content, '', result.tags.join(' ')].join('\n'),
    reply: result.reply,
    source
  })
  if (USE_BACKEND && source === 'fallback') {
...
function fallbackTraining(input: TrainingInput): PlanContentLike {
  ...
  return {
    ...
    phases: [
      {
        name: '第1段 基础建立',
        duration: `${seg}周`,
        items: ['呼吸模式重建', '中立位与核心感知', '基础体式规范']
      },
      {
        name: '第2段 能力提升',
        duration: `${seg}周`,
        items: [
          focus0 ? `${focus0}针对性练习` : '目标部位强化',
          '动作串联与流畅度',
          '小负荷力量导入'
        ]
      },
    ...
    cautions: [
      ...
      input.risks && input.risks !== '无特殊风险' && input.risks
        ? `存在「${input.risks}」情况，请先经医疗专业人员评估后再执行本计划`
        : '如有不适请及时与教练沟通调整'
    ]
  }
}
...
export async function generateTrainingPlan(
  input: TrainingInput
): Promise<{ content: PlanContentLike; source: 'llm' | 'fallback'; warning?: string }> {
  const store = useAiConfigStore()
